  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-ink/45">Gateway Console</p>
          <h1 className="font-[var(--font-sora)] text-3xl font-semibold">Developer Dashboard</h1>
        </div>
        <Link href="/dev">
      ...
      <Card className="mt-5 p-6">
        ...
        <input
          className="mt-3 h-10 w-full rounded-xl border border-ink/15 px-3 font-mono text-sm"
      ...
      <Card className="mt-4 p-6">
        ...
        <form className="mt-3 grid gap-3 md:grid-cols-2" onSubmit={registerMerchant}>
          <input name="name" className="h-10 rounded-xl border border-ink/15 px-3" placeholder="Nexa Store" required />
          <input
            name="business_name"
      ...
      {merchant ? (
        <Card className="mt-4 p-6">
          <p className="text-xs uppercase tracking-[0.25em] text-ink/45">Merchant Credentials</p>
          <p className="mt-2 text-sm">Merchant: <b>{merchant.merchant_id}</b></p>
          <p className="mt-2 break-all rounded-lg bg-ink px-3 py-2 font-mono text-sm text-paper">{merchant.api_key}</p>
          <p className="mt-2 text-xs text-ink/65">Store this key securely. It can create charges, refunds, and payouts.</p>
        </Card>
      ) : null}
      ...
      <Card className="mt-4 p-6">
        ...
        <form className="mt-3 grid gap-3 md:grid-cols-2" onSubmit={createIntent}>
          <input name="amount" type="number" min="1" className="h-10 rounded-xl border border-ink/15 px-3" placeholder="42000" required />
          <input name="currency" className="h-10 rounded-xl border border-ink/15 px-3" placeholder="TND" defaultValue="TND" />
          <input name="customer_name" className="h-10 rounded-xl border border-ink/15 px-3" placeholder="Customer name" />
          <input name="customer_email" className="h-10 rounded-xl border border-ink/15 px-3" placeholder="customer@email.tn" />
          <input name="description" className="h-10 rounded-xl border border-ink/15 px-3 md:col-span-2" placeholder="Order #42" />
          <input
            name="idempotency_key"
            className="h-10 rounded-xl border border-ink/15 px-3 md:col-span-2"
      ...
      {intent ? (
        <Card className="mt-4 p-6">
          ...
          <a className="mt-3 inline-block text-sm font-semibold text-tide underline" href={intent.checkout_url} target="_blank">
            Open Checkout Page
          </a>
        </Card>
      ) : null}
